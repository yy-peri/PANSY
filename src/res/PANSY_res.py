...
from src.res.Encoder import convnextv2_atto
from src.res.Fusion import Fusion_Block
...
class TCR_pMHC_binding(nn.Module):
    ...
    def forward(self, map1, map2, graph_data):
        g_vec = self.graph_encoder(graph_data).to(map1.device)
        feat1, feat2 = self.dual_encoder(map1, map2, g_vec)
        fused_vec, dist_pred, contact_logit = self.fusion(feat1, feat2, g_vec)
        logits = self.classifier(fused_vec)
        return logits, dist_pred, contact_logit
