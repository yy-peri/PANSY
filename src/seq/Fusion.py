...
class Enhancement_texture_LDC(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=False):
        ...
        self.learnable_mask = nn.Parameter(torch.ones_like(self.conv.weight[:, :, 0, 0]), requires_grad=True)  # [out, in]
    ...
    def forward(self, x):
        ...
        # 计算 mask
        center_mask_weight = self.center_mask * normalized_weight.sum(2).sum(2)[:, :, None, None]  # [out, in, 1, 1]
...
class Cross_layer(nn.Module):
    ...
    def forward(self, Fuse, x1, x2, g_vec=None):
        ...
        if self.use_graph and g_vec is not None:
            alpha = self.alpha_mlp(g_vec).view(-1, 1, 1, 1)  # [B,1,1,1]
        else:
...
class Fusion_Block(nn.Module):
    def __init__(self, hidden_dim, g_dim=None):
        super().__init__()
    ...
    def forward(self, input1, input2, g_vec=None):
        ...
        fused = self.cross_fusion(F1, F2, g_vec)
        return fused
